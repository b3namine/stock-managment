...
export const getAllOrders = async (event) => {
	try {
		const orders = await knex('ORDERS')
			.join('PRODUCTS', 'ORDERS.PRODUCT_ID', '=', 'PRODUCTS.PRODUCT_NUMBER')
...
export const insertOrder = async (event, data) => {
	try {
		...
		if (!orderData.PRODUCT_ID || !orderData.CLIENT_ID || !orderData.COUNT) {
			console.error('Error');
			return false;
		}
...
export const editOrder = async (event, data) => {
	try {
		const orderData = JSON.parse(data);
		if (!orderData.ID || !orderData.PRODUCT_ID || !orderData.CLIENT_ID || !orderData.COUNT) {
			return;
		}
		await knex("ORDERS")
		...
		const order = await knex('ORDERS')
			...
			.select(
				...
				'PRODUCTS.STOCK',
				'PRODUCTS.CREATED_AT as PRODUCT_CREATED_AT');
		event.returnValue = order.map((order) => beautyJson(order))[0];
